...
std::string FileDialog::save(const std::string& folderpath, const std::string& filename,
                             const std::vector<std::tuple<std::string, std::string>>& filters)
{
    NFD_Init();
    ...
    nfdresult_t result = NFD_SaveDialog(&outPath, filters_, filters.size(), nullptr, filename.c_str());
